        ...
        BeginDrawing();
            ...
            for (int i = 0; i < MAXNODES; i++) {
              ...
              if (isMouseOverNode && isLineStarted && !isNodeLocked) {
                selectedIndex = i;
                isNodeLocked = true;
                printf("start node %d coordinates: %f, %f \n", selectedIndex, nodes[i].position.x, nodes[i].position.y);
              ...
              if (isNodeDrawn && isSpaceFree) {
                ...
                printf("%d coordinates: %f, %f \n", nodeIndex, nodes[nodeIndex].position.x, nodes[nodeIndex].position.y);
                nodeIndex++;
                colorIndex++;
                isNodeDrawn = false;
              }
            ...
            if (isNodeSelected) {
              ...
              DrawCircleV(nodes[selectedIndex].position, nodes[selectedIndex].diameter, nodes[selectedIndex].color);
              DrawText(TextFormat("%d", selectedIndex + 1), nodes[selectedIndex].position.x - 20, nodes[selectedIndex].position.y - 15, 8, BLACK);
            }

            //LINES---------------------
            if (isLineStarted) {
              lines[lineIndex].startNode = nodes[selectedIndex];
              DrawLine(lines[lineIndex].startNode.position.x, lines[lineIndex].startNode.position.y, mousePosition.x, mousePosition.y, BLACK);
            } else if (isLineEnded) {
              lines[lineIndex].endNode = nodes[selectedIndex];
              DrawLine(lines[lineIndex].startNode.position.x, lines[lineIndex].startNode.position.y, lines[lineIndex].endNode.position.x, lines[lineIndex].endNode.position.y, BLACK);
              lineIndex++;
              isLineStarted = false;
              isLineEnded = false;
              isNodeLocked = false;
            ...
            printf("line %d coordinates: %f, %f to %f, %f \n", lineIndex-1, lines[lineIndex-1].startNode.position.x, lines[lineIndex-1].startNode.position.y, lines[lineIndex-1].endNode.position.x, lines[lineIndex-1].endNode.position.y);
            printf("num of lines = %d", numOfLines);

            //draw lines
            for (int k = 0; k < numOfLines; k++) {
              DrawLine(lines[k].startNode.position.x, lines[k].startNode.position.y, lines[k].endNode.position.x, lines[k].endNode.position.y, BLACK);
            }
